...
class MatrixEditor:
    ...
    def __init__(
        ...
        self._matrix: Optional[Matrix2D] = None
    ...
    def _create_matrix(self, rows: int, cols: int, default: Optional[float] = None) -> None:
        ...
        self._matrix = Matrix2D((rows, cols), default)
        self._update_display()
    ...
    def _on_size_change(self, event=None) -> None:
        ...
        try:
            ...
            new_matrix = Matrix2D((new_rows, new_cols), default)
    ...
    def _fill_random(self) -> None:
        ...
        self._matrix = Matrix2D.from_numpy(random_data)
        self._update_display()
    ...
    def _apply_filter(self, filter_name: str, params: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if isinstance(kernel, tuple):
                ...
                self._matrix = Matrix2D.from_numpy(magnitude)
            else:
                # Apply single kernel
    ...
    def _open_matrix(self) -> None:
        ...
        try:
            self._save_undo_state()
            self._matrix = Matrix2D.load(filepath)
    ...
    def _show_noise_generator(self) -> None:
        ...
        def on_generate(new_matrix: Matrix2D):
            self._save_undo_state()
            self._matrix = new_matrix
